...
/*   Updated: 2016/12/14 20:34:03 by rfernand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ope.h"

int		op_lld(t_env *env, int num_player, unsigned int op_args[3], \
unsigned char codage)
{
	...
	if (is_direct(op_args[0], codage, 0))
		...
		value = memory_2_bytes_to_int(env, op_args[0], num_player, 0);
	else
		return (-1);
	...
	instance[env->num_instance]->reg[op_args[1]]) == 0)
