...
/*   Updated: 2016/12/14 20:33:38 by rfernand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ope.h"

int		op_add(t_env *env, int num_player, unsigned int op_args[3], \
	...
	ft_memcpy(env->champion[num_player]->instance[env->num_instance]->
		reg[op_args[2]], int_to_reg(reg_to_int(env->champion[num_player]->
			instance[env->num_instance]->reg[op_args[0]]) +
				reg_to_int(env->champion[num_player]->
					instance[env->num_instance]->reg[op_args[1]])), i * 2);
